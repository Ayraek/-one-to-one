...
async def generate_question(grade: str, topic: str, name: str) -> str:
    prompt = (
        f"Придумай задание в формате живого кейса для продакт-менеджера уровня {grade} "
        f"по теме «{topic}». Обязательно:\n\n"

        f"1) Сложность и формулировка строго под уровень {grade}. Не перегружай для Junior, усложняй для Senior и выше.\n"
        f"2) Суть задания должна раскрывать навык в теме «{topic}».\n\n"

        f"Примеры тем:\n"
        f"• Soft skills — лидерство, обратная связь, конфликты, пунктуальность.\n"
        f"• Стратегия — roadmap, запуск в новый регион, смена модели, масштабирование.\n"
        f"• Метрики — выбор, анализ, юнит-экономика, корреляции.\n"
        f"• Гипотезы — формулировка, A/B, MVP, цикл проверки.\n"
        f"• Маркетинг — каналы, привлечение, воронки, офферы.\n"
        f"• Управление командой — найм, мотивация, делегирование.\n"
        f"• Требования к продукту — сбор, приоритизация, согласование с командой.\n\n"

        f"Контекст должен быть живым: компания, команда, ограничения. "
        f"Не пиши приветствий, обращений и воды. Только текст задания, до 800 символов, без вступлений."
    )
