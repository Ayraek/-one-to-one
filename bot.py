...
TOPICS = [
    "Кейсы (решение кейса)",
    "Метрики (задачи, связанные с метриками)",
    "Команда разработки (решение задачи по работе с командой)",
    "Софт скиллы (решение конфликтов)",
    "Инструменты (работа с инструментами для продакта)",
    "Исследования (задачи, связанные с гипотезами и экспериментами)"
]
...
async def handle_grade_selection(callback: types.CallbackQuery, state: FSMContext):
    ...
    # Выбираем тему в режиме round-robin:
    data = await state.get_data()
    last_topic_index = data.get("last_topic_index", -1)
    next_topic_index = (last_topic_index + 1) % len(TOPICS)
    topic = TOPICS[next_topic_index]
    await state.update_data(last_topic_index=next_topic_index)
    
    # Генерация задания с учётом выбранной темы
...
async def generate_question(grade: str, topic: str) -> str:
    """
    Генерируем задание по конкретной теме.
    Если сгенерированный текст больше 4000 символов, повторяем попытку (до 3 раз).
    Не используй звёздочки — вместо них применяй эмодзи.
    """
    max_chars = 4000
    attempts = 3
    prompt = (
        f"Ты опытный продакт-менеджер и консультант по продуктовой стратегии. "
        f"Сгенерируй задание (кейс) для продакт-менеджера уровня {grade} по теме: «{topic}». "
        "В задании опиши реальную бизнес-ситуацию, вызовы и задачи, связанные с данной темой. "
        "Удели внимание аналитике, стратегическим решениям и аргументации. "
        "Используй эмодзи (например, 🔹, 🚀, 💡) для выделения ключевых пунктов. "
        "Сделай задание кратким и ёмким, чтобы оно не превышало 4000 символов. "
        "Основной акцент – рассуждения, анализ и аргументация."
    )
    for attempt in range(attempts):
        try:
                ...
                messages=[
                    {
                        ...
                        "content": (
                            ...
                            "Вместо звёздочек применяй эмодзи для выделения ключевых моментов. "
                            "Текст должен быть полным и не обрываться, но его длина не должна превышать 4000 символов."
                        )
                    },
                    {
                        "role": "user",
            ...
            if len(question_text) <= max_chars:
                ...
                logging.info(f"Сгенерированный вопрос слишком длинный ({len(question_text)} символов), повторная попытка {attempt+1}")
        except Exception as e:
            logging.error(f"Ошибка генерации вопроса: {e}")
            return "❌ Ошибка генерации вопроса."
    return "❌ Не удалось сгенерировать подходящий вопрос, попробуйте позже."
...
async def generate_correct_answer(question: str, grade: str) -> str:
    prompt = (
        ...
        "Не используй звёздочки, используй эмодзи или обычный текст для выделения ключевых пунктов."
    )
    try:
        response = await asyncio.to_thread(
            client.chat.completions.create,
