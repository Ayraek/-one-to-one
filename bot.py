...
async def generate_question(grade: str, topic: str) -> str:
    ...
    Если сгенерированный текст превышает 4000 символов, делаем до 3 попыток.
    Не используй символ * – вместо него эмодзи.
    """
    max_chars = 4000
    attempts = 3
    prompt = (
        ...
        "Опиши реальную бизнес-ситуацию, вызовы и задачи, связанные с данной темой. "
        "Удели внимание аналитике, стратегическим решениям и аргументации. "
        "Используй эмодзи (например, 🔹, 🚀, 💡) для выделения ключевых моментов. "
        "Сделай текст емким и не превышающим 4000 символов. "
        "Основной акцент – на рассуждениях, анализе и аргументации."
    )
    for attempt in range(attempts):
        try:
            response = await asyncio.to_thread(
                ...
                messages=[
                    {"role": "system", "content": (
                        "Ты генерируешь кейсы для продакт-менеджеров. Не используй символ *, вместо этого применяй эмодзи. "
                        "Текст должен быть полным и не превышать 4000 символов."
                    )},
            ...
            if len(question_text) <= max_chars:
                ...
                logging.info(f"Сгенерированный вопрос длиннее лимита ({len(question_text)} символов). Попытка {attempt+1}")
