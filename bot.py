...
async def evaluate_answer(question: str, student_answer: str, student_name: str) -> str:
    prompt = (
        ...
        f"Проанализируй ответ студента {student_name} по следующим критериям:\n"
        ...
        "После оценки по каждому критерию, укажи итоговую оценку (Score) от 0 до 1 и предоставь подробный обратный фидбэк.\n"
        "ВАЖНО: ОБЯЗАТЕЛЬНО ВЫВЕДИ СТРОКУ, НАЧИНАЮЩУЮСЯ С 'Score:' с числовым значением (например, Score: 0.5). Если оценка не может быть определена, выведи Score: 0.0.\n\n"
        "Ответ должен быть строго в следующем формате (без лишних комментариев и пояснений):\n\n"
        "Критерии:\n"
