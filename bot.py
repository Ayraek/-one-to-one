...
async def handle_task_answer(message: Message, state: FSMContext):
    ...
    try:
        clarification_response = await asyncio.to_thread(
            ...
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "Ты опытный интервьюер. Анализируешь сообщение кандидата: если это запрос о дополнительной информации — отвечаешь конкретно и по-человечески. Если это уже полноценный ответ — не отвечай вообще."},
