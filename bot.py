...
async def handle_task_answer(message: Message, state: FSMContext):
    ...
    if message.text.strip().endswith("?"):
        clarification_prompt = (
            f"Ты — опытный интервьюер-продакт из Яндекса или VK. Сейчас проводишь собеседование с кандидатом.\n\n"
            f"Вот основной вопрос:\n{question}\n\n"
            f"Кандидат спрашивает:\n{message.text.strip()}\n\n"
            f"Пожалуйста, ответь на его уточняющий вопрос прямо, от первого лица. Можешь добавить пример продукта, если это поможет.\n"
            f"Формулируй как в диалоге: обращайся по имени или на 'ты'. Без воды, но дружелюбно и понятно.\n"
            f"Если уточнение не по теме — мягко верни фокус к заданию.\n"
    ...
    messages=[
        ...
        {"role": "user", "content": clarification_prompt}
            ...
            await message.answer("🤖 Не удалось обработать вопрос. Попробуй ещё раз.")
            return
