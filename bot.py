...
async def evaluate_answer(question: str, student_answer: str, student_name: str) -> str:
    prompt = (
        ...
        "После оценки по каждому критерию, укажи итоговую оценку (Score) от 0 до 1 и дай подробный обратный фидбэк.\n"
        "Ответ должен быть строго в следующем формате (без лишних комментариев):\n\n"
        "Критерии:\n"
    ...
    try:
        response = await asyncio.to_thread(
            ...
            max_tokens=450,
            temperature=0.3
            # Параметр stop удалён, чтобы не прерывать генерацию нужного шаблона
        )
        return response.choices[0].message.content.strip()
    except Exception as e:
