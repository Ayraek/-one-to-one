...
async def generate_question(grade: str, topic: str) -> str:
    ...
    Используем повышенный max_tokens, чтобы избежать обрезания.
    ...
    prompt = (
        ...
        "Текст должен быть подробным, полным и не обрываться в конце. "
        "Основной акцент сделай на том, чтобы студент рассуждал, анализировал и аргументировал свои решения."
    )
    try:
            ...
            messages=[
                {
                    "role": "system",
                    "content": (
                        "Ты генерируешь кейсы для продакт-менеджеров. Не используй символ *, а применяй эмодзи для выделения ключевых моментов. "
                        "Текст должен быть полным и не обрываться."
                    )
                },
                {
                    "role": "user",
