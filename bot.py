...
async def generate_question(grade: str) -> str:
    prompt = (
        ...
        "Кейс должен включать описание бизнес-ситуации, в которой необходимо принять решения, связанные с анализом метрик, управлением командой, проведением исследований, рациональным использованием ресурсов, запуском MVP и проверкой гипотез через эксперименты. "
        "Задание должно побуждать студента к глубоким рассуждениям, анализу и аргументации, демонстрируя базу знаний продуктовой практики, а не просто проверять знание правильного ответа. "
        "Опиши все ключевые аспекты, которые студенту следует учесть при формировании ответа."
    )
    try:
        response = await asyncio.to_thread(
            ...
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "Ты генерируешь кейсы для продакт-менеджеров."},
