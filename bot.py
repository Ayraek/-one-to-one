...
async def generate_question(grade: str, topic: str) -> str:
    """
    Генерирует прикладной кейсовый вопрос для собеседования по теме и уровню.
    """
    prompt = (
        f"Ты — опытный интервьюер по найму продакт-менеджеров в ведущих российских ИТ-компаниях "
        f"(например, Яндекс, Сбер, VK, Ozon, Тинькофф). "
        f"Сейчас ты проводишь онлайн-собеседование с кандидатом на позицию продакт-менеджера уровня {grade} "
        f"в компании типа B2C. Задай один реалистичный прикладной кейсовый вопрос по теме: {topic}.\n\n"
        f"Формулируй вопрос на основе конкретной ситуации в продукте или бизнесе, как будто ты действительно "
        f"оцениваешь кандидата в реальной задаче. Используй реальный контекст, например: проблемы с метриками, "
        f"конфликт в команде, необходимость приоритизации, пользовательские жалобы и т.д.\n\n"
        f"Не используй слово 'Вопрос'. Просто напиши сам текст. Без приветствия, без инструкций.\n"
        f"Без Markdown и *звёздочек*. Без лишней строгости, но профессионально."
    )
    ...
    try:
        response = await asyncio.to_thread(
            ...
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "Ты опытный интервьюер в крупных ИТ-компаниях. Задаёшь кейсовые вопросы на собеседовании."},
