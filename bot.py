...
async def generate_question(grade: str, topic: str) -> str:
    """
    Генерирует прикладной, реалистичный вопрос с цифрами и контекстом для собеседования.
    """
    prompt = (
        f"Ты — опытный интервьюер по найму продакт-менеджеров в ведущих российских ИТ-компаниях "
        f"(Яндекс, VK, Ozon, Сбер, Тинькофф). Сейчас ты проводишь онлайн-собеседование с кандидатом уровня {grade} "
        f"в компании формата B2C. Тема: {topic}.\n\n"
        f"Сформулируй 1 прикладный вопрос с контекстом: как будто ты реально общаешься с кандидатом. "
        f"Добавь конкретику: цифры, метрики, реальные ситуации, например:\n"
        f"«В нашем приложении количество регистраций упало на 15% за 2 недели, что ты сделаешь?»\n"
        f"или\n"
        f"«Retention D7 снизился с 32% до 18%, а churn вырос — какие гипотезы?»\n\n"
        f"Требования:\n"
        f"- Не используй фразу 'Вопрос:'\n"
        f"- Без пояснений, просто сам вопрос\n"
        f"- Можно использовать эмодзи, если это органично (📉, 📈, 🤔 и т.п.)\n"
        f"- Тон — живой, как в реальной беседе, без чрезмерной строгости\n"
    ...
    try:
        response = await asyncio.to_thread(
            ...
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "Ты собеседуешь кандидатов на продакт-менеджеров. Спрашиваешь живо, по делу, с цифрами."},
                {"role": "user", "content": prompt}
            ],
            max_tokens=300,
            temperature=0.8  # чуть выше — для большей креативности
        )
