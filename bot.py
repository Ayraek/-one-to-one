...
async def process_clarification(message: Message, state: FSMContext):
    ...
    clarification_prompt = (
        ...
        "Если вопрос непонятен — мягко уточни. Если просит пример — просто дай направление и не отвечай за него на вопрос, просто направь. "
        "Если путается — переформулируй задание. Без приветствий, сразу к сути."
        "- Помочь студенту лучше понять суть задания."
        " Переформулировать задание проще, если нужно."
        "НЕ раскрывать готовый ответ."
        "НЕ подсказывать, как именно решать."
        "Если вопрос не по теме — мягко вернуть к сути."
    )
...
async def generate_question(grade: str, topic: str, name: str) -> str:
    """
    Генерирует разнообразное живое задание по грейду и теме.
    • Привязка к разным индустриям и форматам компаний
    • Обязательный контекст (команда, бюджет, сроки)
    • Каждый кейс — новый, без повторов
    • Только текст задания, до 800 символов
    """
    prompt = f"""
Придумай **уникальный**, реалистичный кейс для продакт-менеджера уровня {grade} по теме «{topic}». Обязательно:
1. Укажи компанию (стартап, В2С/В2В, финтех, edtech, e-commerce и т.п.).
2. Опиши команду (размер, роли), бюджет/время или другие ограничения.
3. Сформулируй конкретную задачу, отражающую навык в теме «{topic}» на уровне {grade}.
4. Не повторяй примеры из системы или предыдущих заданий — каждый кейс должен быть **новым**.
5. Ответ дай только текстом задания (до 800 символов), без приветствий и списков «Пожалуйста, сделайте…».

Пример (не копировать дословно):
• Компания: мобильный финтех-стартап, команда из 3 PM, 2 дизайнеров, 2 разработчиков; бюджет на MVP \$20 000.
• Задача: спроектировать flow для onboarding, чтобы повысить активацию на 15%.
    """
        ...
        response = await asyncio.to_thread(
            ...
            messages=[
                {"role": "system", "content": (
                    ...
                    "Каждый раз сценарий должен быть новым: другая индустрия, другие ограничения."
                )},
